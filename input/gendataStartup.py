...
if True:
  runname = 'StraightSlopeUpsidedownStrat003Dye001'
  comments = "55 km wide slope (subcritical?) restart from 003, add salt as passive tracer; 1200 m deep StraightSlopeUpsidedownStrat003Dye"
  ...
  with xm.open_mdsdataset('../results/StraightSlopeUpsidedownStrat003/input/', iters='all', endian='<', prefix=['spinup', 'spinup2d']) as ds:
    ds = ds.isel(time=-1, j=0, j_g=0)
    with open(indir+"/Tinit.bin", "wb") as f:
      ds.THETA.to_numpy().tofile(f)
    with open(indir+"/Uinit.bin", "wb") as f:
      ds.UVEL.to_numpy().tofile(f)
    with open(indir+"/Etainit.bin", "wb") as f:
      ds.ETAN.to_numpy().tofile(f)

    S = ds.THETA.to_numpy() * 0 + 35
    ind = np.where(ds.XC>110_000)[0][0]
    tt = ds.THETA.to_numpy()[:, ind]
    indk = np.where(tt>0)[0][-1]
    for i in range(50):
        for j in range(60):
            if indk-j > 0:
  ...
  fig, ax = plt.subplots()
  ax.pcolormesh(S, rasterized=True)
  fig.savefig(outdir+'/figs/S.pdf')
