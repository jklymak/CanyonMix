...
### Run only one or more specific methods:

```bash
# Run only Eulerian means and mooring time series
python generate_figures.py --runname Slope2D093 --mooringX -55 -45 -35 \
   --method eulerian_means timeseries_moorings

# Run only streamfunction and Hovmoller (requires layered data)
python generate_figures.py --runname Slope2D093 --mooringX -55 -45 -35 \
   --method streamfunction hovmoller
```
...
- `--method`: (Optional) One or more methods to run. Default is `all`.
